...
/*   Updated: 2024/07/09 11:25:07 by mbankhar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static size_t	ft_lens(const char *c)
{
	size_t	i;
...
char **ft_splitspecial(char const *s)
{
	...
	while (i < counter)
	{
		...
		arr[i] = (char *)malloc(sizeof(char) * (ft_lens(&s[p]) + 1));
		if (!ft_malloc_check(arr, i))
		{
			return (NULL);
		}
		ft_strlccpy(arr[i], &s[p], ft_lens(&s[p]) + 1);
		i++;
